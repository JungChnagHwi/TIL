...
const API_KEY = "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"
...
const chatReceive = (userMsg) => {
  ...
  .then( res => {
    ...
    response = res.data.choices[0].message.content //입력했었어!

    // 2. 챗팅창에 메세지를 등록한다.
    addChat("receive", response)  //주석해제
     

    // 3. 챗팅의 연속성을 위해 이전 메세지를 oldMsg 변수에 저장
    // messages 에서 system 메세지의 content 값으로 설정됨
    oldMsg = response //입력했었어!
